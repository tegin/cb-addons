...
class TestAssetManagementDepreciationPercentage(TransactionCase):
    ...
    def test_1_percentage_days_calc(self):
        ...
        self.assertAlmostEqual(
            asset.depreciation_line_ids[2].amount, 28 * day_amount, places=2
        )
        # March has 29 days
        self.assertAlmostEqual(
            asset.depreciation_line_ids[3].amount, 31 * day_amount, places=2
        )
        # April has 30 days
        self.assertAlmostEqual(
            asset.depreciation_line_ids[4].amount, 30 * day_amount, places=2
        )
        # January has 31 days
        self.assertAlmostEqual(
            asset.depreciation_line_ids[13].amount, 31 * day_amount, places=2
        )
        # February has 29 days
        self.assertAlmostEqual(
            asset.depreciation_line_ids[14].amount, 29 * day_amount, places=2
        )
        # March has 29 days
        self.assertAlmostEqual(
            asset.depreciation_line_ids[15].amount, 31 * day_amount, places=2
        )
        # April has 30 days
        self.assertAlmostEqual(
            asset.depreciation_line_ids[16].amount, 30 * day_amount, places=2
        )
