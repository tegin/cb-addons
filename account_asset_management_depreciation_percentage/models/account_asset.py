...
class AccountAssetAsset(models.Model):
    ...
    def _get_year_amount_linear(self):
        if self.method_time == 'percentage':
            return self.depreciation_base * self.annual_percentage/100
        return super(AccountAssetAsset, self)._get_year_amount_linear()
    ...
    def _get_depreciation_stop_date(self, depreciation_start_date):
        ...
        if self.method_time == 'percentage':
            number = 0
            percentage = 100.0
            stop_date = depreciation_start_date.date()
            while percentage > 0:
                ...
                number += 1
            return datetime.combine(stop_date, datetime.min.time())
