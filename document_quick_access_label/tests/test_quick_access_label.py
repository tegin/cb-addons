...
class TestQuickAccessLabel(TransactionCase):
    ...
    def test_views_no_button(self):
        ...
        self.assertFalse(any(
            json.loads(button.attrib['context']).get(
                'rule_id', False
            ) == self.rule.id for button in buttons
        ))
    ...
    def test_views_button_no_options(self):
        ...
        for button in buttons:
            if json.loads(button.attrib['context']).get(
                'rule_id', False
            ) == self.rule.id:
                final_button = button
                break
    ...
    def test_views_button_options(self):
        ...
        self.assertEqual(
            final_button.attrib['attrs'],
            self.rule.label_attrs
        )
