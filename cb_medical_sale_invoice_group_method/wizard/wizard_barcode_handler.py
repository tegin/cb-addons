...
class WizardSalePreinvoiceGroup(models.TransientModel):
    ...
    status = fields.Text(readonly=1, default="Start scanning")
    status_state = fields.Integer(default=0, readonly=1)
    encounter_id = fields.Many2one(
        "medical.encounter", computed="on_barcode_scanned"
    )
    processed_lines = fields.Many2many(
        "sale.order.line", computed="on_barcode_scanned"
    )
    ...
    def on_barcode_scanned(self, barcode):
        ...
        self.processed_lines = self.line_ids.filtered(
            lambda r: r.encounter_id.id == self.encounter_id.id
