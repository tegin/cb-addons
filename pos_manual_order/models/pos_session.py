...
class PosSession(models.Model):
    ...
    def _get_manual_order_vals(
        ...
        return {
            ...
            'lines': [
                (0, 0, r) for r in self._get_manual_order_line_vals(
    ...
    def _get_manual_order_line_vals(self, product, qty, price, discount):
        return [{
            ...
            'tax_ids': [(4, tx.id) for tx in product.taxes_id.filtered(
    ...
    def _add_manual_order(
        ...
        if self.state != 'opened':
            raise ValidationError(_(
                'Manual Order can only be added on in progress sessions'
            ))
