...
class PosSessionAddOrder(models.TransientModel):
    ...
    @api.depends(
        'product_id', 'session_id.config_id.company_id', 'partner_id',
        'price', 'discount',
    )
    ...
    def run(self):
        return self.session_id._add_manual_order(
            self.product_id, self.qty, self.price, self.discount,
            self.partner_id, self.fiscal_position_id, self.journal_id
        )
