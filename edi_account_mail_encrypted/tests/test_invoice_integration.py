...
class TestInvoiceIntegration(TransactionCase):
    ...
    def setUp(self):
        ...
        self.partner = self.env['res.partner'].create({
            ...
            'email_integration_password': self.env[
                'account.invoice.integration.log'
            ]._encrypt_value('TEST PASSWORD'),
            'oficina_contable': '1',
            'invoice_integration_method_ids': [(6, 0, [
                self.env.ref('invoice_integration_email.integration_email').id
            ])]
        })
    ...
    def test_encryption(self):
        passwrd = 'TEST PASSWORD TEST'
        self.env['res.encrypt.value'].create({
            'model': self.partner._name,
            'res_id': self.partner.id,
            'value': passwrd,
            'field': 'email_integration_password',
        }).doit()
        val = self.env['res.view.value'].with_context(
            default_model=self.partner._name,
            default_res_id=self.partner.id,
            default_field='email_integration_password'
        ).create({})
        self.assertEqual(passwrd, val.value)
