    ...
    def send_email(self):
        try:
            ...
            values = (
                ...
                .generate_email(invoice.ids, ["subject", "body_html"])[
                    invoice.id
                ]
            )
            msg = IrMailServer.build_email(
                email_from=self._get_email(invoice.company_id.partner_id)[0],
            ...
            self.integration_id.write({"state": "sent", "can_send": False})
        except Exception:
            buff = StringIO()
            traceback.print_exc(file=buff)
            _logger.error(buff.getvalue())
            self.state = "failed"
            self.integration_id.write({"state": "failed"})
