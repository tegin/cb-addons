...
class SaleOrder(models.Model):
    ...
    third_party_move_id = fields.Many2one(
        comodel_name="account.move", string="Third party move", readonly=True
    )
    source_third_party_order_id = fields.Many2one(
        comodel_name="sale.order", string="Third party order", readonly=True
    )
    ...
    def _compute_third_party_residual(self):
        ...
        for rec in self:
            ...
            for line in rec.sudo().third_party_move_id.line_ids:
                if (
                    line.account_id == third_party_customer_account
                    ...
                    if line.currency_id == rec.currency_id:
                        ...
                        in_residual += from_currency.compute(
                            line.amount_residual, rec.currency_id
                        )
                elif (
                    line.account_id == third_party_supplier_account
    ...
    def _third_party_move_vals(self):
        ...
        if not third_party_customer_account:
            raise UserError(
                _(
                    "Please define a third party customer account "
        ...
        if not third_party_supplier_account:
            raise UserError(
                _(
                    "Please define a third party supplier account "
