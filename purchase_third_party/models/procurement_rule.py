...
class ProcurementRule(models.Model):
    ...
    def _run_buy(self, product_id, product_qty, product_uom, location_id, name,
        ...
        for line in po.order_line.filtered(
            lambda r: (
                ...
                r.product_uom == product_id.uom_po_id)
        ):
            if line._merge_in_existing_line(
                ...
                vals = self._update_purchase_order_line_supplier(
                    product_id, product_qty, product_uom, values, line,
                    supplier)
    ...
    def _prepare_purchase_order_line(
        ...
        if seller.third_party_partner_id and not po.third_party_order:
            return UserError(_(
                'The partner of the third party must be the same'
            ))
        ...
        price_unit = self.env['account.tax']._fix_tax_included_price_company(
            seller.third_party_price, product_id.supplier_taxes_id, taxes_id,
            values['company_id']) if seller else 0.0
        if (
            price_unit and seller and po.currency_id and
