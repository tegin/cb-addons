...
class MedicalCareplan(models.Model):
    ...
    def _add_request_group(
        self, service=False, qty=1, order_by=False, authorization_number=False,
        performer=False, **kwargs
    ):
        self.ensure_one()
        if not service:
        ...
        call = self.env['medical.careplan.add.plan.definition'].with_context(
            default_careplan_id=self.id,
        ).create({
