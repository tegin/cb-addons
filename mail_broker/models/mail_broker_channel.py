...
class MailBrokerChannel(models.Model):
    ...
    message_ids = fields.One2many(
        "mail.message.broker", inverse_name="channel_id",
    )
    ...
    broker_token = fields.Char(
        related="broker_id.token", store=True, required=False
    )
    ...
    def message_fetch(self, domain=False, limit=30):
        ...
        return (
            ...
            .search(
                [("broker_channel_id", "=", self.id)] + domain, limit=limit
            )
            .message_format()
        )
    ...
    def telegram_message_post_broker(self, body=False, **kwargs):
        # TODO: change the function on the external broker, DELETE THIS AFTER
        return self.message_post_broker(
            body=body, broker_type="telegram", **kwargs
        )
