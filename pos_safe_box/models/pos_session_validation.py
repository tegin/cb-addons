...
class PosSessionValidation(models.Model):
    ...
    def _compute_statement_values(self):
        for record in self:
            ...
            record.cash_amount = sum(statements.filtered(
    ...
    def account_move_vals(self, statement):
        ...
        if not account:
            raise ValidationError(_(
                'Account cannot be found for this company'))
    ...
    def close(self):
        ...
        for key in lines.keys():
            ...
            self.env['safe.box.move.line'].create(self.safe_box_move_line_vals(
                self.closing_move_id, safe_box, lines[key]))
        for statement in self.statement_ids.filtered(
            lambda r: (
                r.journal_id.type == 'cash' and
                r.total_entry_encoding != 0
            )
        ):
    ...
    def approve(self):
        ...
        for initial_safe_box, end_safe_box in [
            ...
            if end_safe_box:
                ...
                lines.append({
                    'safe_box_id': end_safe_box.id,
                    'amount': value,
                })
                lines.append({
                    'safe_box_id': initial_safe_box.id,
                    'amount': -value,
                })
        if len(lines) > 0:
