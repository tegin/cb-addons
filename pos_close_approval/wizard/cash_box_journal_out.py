...
class CashBoxJournalOut(models.TransientModel):
    _name = 'cash.box.journal.out'
    ...
    def _default_can_edit(self):
        return self._default_value(self.default_can_edit)
    ...
    def default_account(self, active_model, active_ids):
        return self.default_company(
            active_model, active_ids).transfer_account_id

    def default_can_edit(self, active_model, active_ids):
        if active_model == 'pos.session':
            active = self.env[active_model].browse(active_ids)
            if active.state == 'pending_approval':
                return True
        return False
    ...
    def run(self):
        ...
        if active_model == 'pos.session':
            ...
            return super(CashBoxJournalOut, self).run()
