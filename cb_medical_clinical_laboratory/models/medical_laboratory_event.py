...
class MedicalLaboratoryEvent(models.Model):
    ...
    def _check_code(self):
        if self.filtered(lambda r: (
            r.laboratory_service_id and
            r.laboratory_service_id.laboratory_code != r.laboratory_code
        )):
    ...
    def _onchange_laboratory_service(self):
        for rec in self:
            ...
            cov = rec.laboratory_request_id.careplan_id.coverage_id.\
            ...
            if rec.laboratory_service_id in \
                ...
                if cai.coverage_percentage < 100:
                    ...
                    rec.private_amount = price.amount * (
                        100 - cai.coverage_percentage) / 100
                    rec.private_cost = price.cost * (
                        100 - cai.coverage_percentage) / 100
                else:
                    rec.is_sellable_private = False
                    rec.private_amount = 0
                ...
                raise ValidationError(_(
                    'Laboratory service is not covered.'
                ))
