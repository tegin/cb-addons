...
class HrHolidays(models.Model):
    ...
    time_description = fields.Char(
        string="Time", compute="_compute_time_description", readonly=True
    )
    ...
    def _inverse_date_from_full(self):
        for record in self.filtered(
            ...
            tz_name = record.employee_id.user_id.tz or record.env.user.tz
            dt = (
                fields.Datetime.from_string(record.date_from_full)
                .replace(
                    hour=0,
                    minute=0,
                    second=0,
    ...
    def _inverse_date_to_full(self):
        for record in self.filtered(
            ...
            tz_name = record.employee_id.user_id.tz or record.env.user.tz
            dt = (
                fields.Datetime.from_string(record.date_to_full)
                .replace(
                    hour=23,
                    minute=59,
                    second=59,
    ...
    def _check_holidays(self):
        super(
            HrHolidays, self.filtered(lambda r: not r.count_in_hours)
        )._check_holidays()
    ...
    @api.depends(
        "holiday_status_id", "number_of_days_temp", "number_of_hours_temp"
    )
    def _compute_time_description(self):
        for record in self:
            if record.count_in_hours:
                record.time_description = (
                    "%.2f hour(s)" % record.number_of_hours_temp
                )
            else:
                record.time_description = (
                    "%.2f day(s)" % record.number_of_days_temp
                )
    ...
    def name_get(self):
        res = super(
            HrHolidays,
            self.filtered(
                lambda r: not r.count_in_hours
                and r.holiday_status_id
                and r.employee_id
            ),
        ).name_get()

        for leave in self.filtered(
            lambda r: r.count_in_hours
            and r.holiday_status_id
            and r.employee_id
        ):
            res.append(
    ...
    def _prepare_create_by_category(self, employee):
        ...
        values.update({"number_of_hours_temp": self.number_of_hours_temp})
        return values
