...
class HashSearch(models.Model):
    ...
    def cron_attach_documents(self, limit=False, path=False):
        ...
        for element in elements:
            _logger.info('Processing %s' % element)
            try:
                result = self._search_document(element)
                self._postprocess_document(element, result)
            except OCRException:
                _logger.info('Element %s was corrupted' % element)
                os.unlink(element)
        return True
    ...
    def _search_pil_image(self, image):
        ...
        for result in results:
            hashes |= self.search([('name', '=', result.data.decode('utf-8'))], limit=1)
        _logger.info('Found hashes %s' % hashes.ids)
        return hashes
