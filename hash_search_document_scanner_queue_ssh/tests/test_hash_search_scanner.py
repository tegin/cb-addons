...
PATCH_CLASS = 'odoo.addons.hash_search_document_scanner_queue_ssh.models.' \
...
class TestHashSearchScanner(TransactionCase):
    ...
    def setUp(self):
        ...
        self.env['ir.config_parameter'].set_param(
            'hash_search_document_scanner.path',
            self.tmpdir.name
        )
        self.env['ir.config_parameter'].set_param(
        ...
        self.env['ir.config_parameter'].set_param(
            'hash_search_document_scanner.ok_path',
            self.ok_tmpdir.name
        )
        self.env['ir.config_parameter'].set_param(
            'hash_search_document_scanner.failure_path',
    ...
    def test_ok_pdf(self, mck):
        ...
        self.assertFalse(
            os.path.exists(
                os.path.join(self.pre_tmpdir.name, 'test_file.pdf'))
        )
        self.assertFalse(
            os.path.exists(
