odoo.define('web.OrgChart', function (require) {
    ...
    var FieldDepartmentChart = AbstractField.extend({
        ...
        _render: function () {
            ...
            return this._getOrgData(this.recordData.id).then(function () {
                self.$el.html(QWeb.render(
                    "hr_department_chart", self.orgData));
                self.$('[data-toggle="popover"]').each(function () {
                    $(this).popover({
                        html: true,
                        title: function () {
                            var $title = $(QWeb.render(
                                'hr_orgchart_emp_popover_title', {
                                    department: {
                            ...
                            $title.on('click',
                                '.o_department_redirect', _.bind(
                                    self._ondepartmentRedirect, self));
                            return $title;
                        },
                        container: 'body',
                        placement: 'left',
                        ...
                        content: function () {
                            var $content = $(QWeb.render(
                                'hr_orgchart_emp_popover_content', {
                                    department: {
                                        id: $(this).data('emp-id'),
                                        name: $(this).data('emp-name'),
                                        direct_sub_count: parseInt(
                                            $(this).data('emp-dir-subs'), 10),
                                        indirect_sub_count: parseInt(
                                            $(this).data('emp-ind-subs'), 10),
                                    },
                                }));
                            $content.on('click',
                                '.o_department_sub_redirect', _.bind(
                                    self._ondepartmentSubRedirect, self));
                            return $content;
                        },
                        template: $(QWeb.render(
                            'hr_orgchart_emp_popover', {})),
                    });
                });
            });
        },
        ...
        _ondepartmentRedirect: function (event) {
            ...
            var department_id = parseInt($(event.currentTarget).data(
                'department-id'), 10);
        ...
        _ondepartmentSubRedirect: function (event) {
            ...
            var department_id = parseInt(
                $(event.currentTarget).data('department-id'), 10);
            var department_name = $(
                event.currentTarget).data('department-name');
            var type = $(event.currentTarget).data('type') || 'direct';
            var domain = [['parent_id', '=', department_id]];
            var name = _.str.sprintf(_t(
                "Direct Subordinates of %s"), department_name);
            if (type === 'total') {
                domain = ['&', ['parent_id', 'child_of', department_id],
                    ['id', '!=', department_id]];
                name = _.str.sprintf(
                    _t("Subordinates of %s"), department_name);
                ...
                name = _.str.sprintf(
                    _t("Indirect Subordinates of %s"), department_name);
            }
            if (department_id) {
                return this.do_action({
                    name: name,
